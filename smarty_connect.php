<?php
	...
	function getSmarty($css = array(), $js = array())
	{
		...
		$countSlashSymbols = substr_count(substr($currentDirFromCallScriptFile, 
			strspn($currentdDirFromScriptFile, $currentDirFromCallScriptFile)), '\\');
		...
		$smarty->compile_dir  = $currentdDirFromScriptFile.'/tpl/templates_c/';
		$smarty->config_dir   = $currentdDirFromScriptFile.'/tpl/configs/';
		$smarty->cache_dir    = $currentdDirFromScriptFile.'/tpl/cache/';
		if(!empty($css))
		{
			for($i = 0; $i < count($css); $i++)
			{
				$css[$i] = str_repeat('..\\', $countSlashSymbols).'css\\'.$css[$i].'.css';
			}
		...
		if(!empty($js))
		{
			...
			{
				$js[$i] = str_repeat('..\\', $countSlashSymbols).'javascript\\'.$js[$i].'.js';
			}
			$smarty->assign('js', $js);
		}
